...
{
    S_ASSERT(oc.width*oc.height == row.cols);
...
{
    S_ASSERT(oc.width == oc.height && "We support only square canvases");
    r = gym_fit_square(r);
        ...
        BeginDrawing();
            ...
            gym_layout_begin(GLO_HORZ, gym_root(), 2, 10);
                ...
                gym_layout_begin(GLO_VERT, gym_layout_slot(), 2, 10);
                    ...
                    canvas_to_row(canvas, mat_row(NN_INPUT(nn), 0));
                    nn_forward(nn);
                    {
                        Gym_Rect slot = gym_layout_slot();
                        gym_render_mat_as_heatmap(NN_OUTPUT(nn), slot, NN_OUTPUT(nn).cols);
